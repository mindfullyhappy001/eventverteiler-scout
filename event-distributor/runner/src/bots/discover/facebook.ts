...
export async function discoverFacebook(context: BrowserContext, cfg: any) {
  const page = await context.newPage();
  try {
    await page.goto('https://www.facebook.com/login');
    await page.waitForLoadState('domcontentloaded');
    if (cfg?.email && cfg?.password) {
      try { await page.getByLabel('Email address or phone number').fill(cfg.email); } catch {}
      try { await page.getByLabel('Password').fill(cfg.password); } catch {}
      await clickByText(page, 'Log In').catch(()=>{});
      await page.waitForLoadState('networkidle');
    }
    await page.goto('https://www.facebook.com/events/create').catch(()=>{});
    await page.waitForLoadState('domcontentloaded');
    ...
    map['privacy'] = find(/privacy|sicht|Visibility/i);
