...
export async function discoverEventbrite(context: BrowserContext, cfg: any) {
  const page = await context.newPage();
  try {
    await page.goto('https://www.eventbrite.com/login');
    await page.waitForLoadState('domcontentloaded');
    if (cfg?.email && cfg?.password) {
      try { await page.getByLabel('Email address').fill(cfg.email); } catch {}
      try { await page.getByLabel('Password').fill(cfg.password); } catch {}
      await clickByText(page, 'Log in').catch(()=>{});
      await page.waitForLoadState('networkidle');
    }
    await page.goto('https://www.eventbrite.com/create').catch(()=>{});
    await page.waitForLoadState('domcontentloaded');
    ...
    map['category'] = find(/categor|kategor/i);
