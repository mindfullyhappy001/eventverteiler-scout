...
(async () => {
  ...
  const session = await loadOrCreateSession(context, 'eventbrite');
  ...
  try {
    ...
    if (cfg.email && cfg.password) {
      await smartFill(page, 'Email address', cfg.email) || await smartFill(page, 'E-Mail', cfg.email);
      await smartFill(page, 'Password', cfg.password) || await smartFill(page, 'Passwort', cfg.password);
      await clickByText(page, 'Sign in') || await clickByText(page, 'Anmelden');
      await page.waitForLoadState('networkidle');
    }
    ...
    if (event.title) await smartFill(page, 'Event title', event.title);
    ...
    await page.goto('https://www.eventbrite.com/myevents/');
    const visible = await page.getByText(event.title, { exact: false }).first().isVisible().catch(() => false);
    if (!visible) throw new Error('Verification failed: event title not visible on Eventbrite my events');
